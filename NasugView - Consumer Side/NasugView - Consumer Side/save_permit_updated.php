...
try {
    ...
    try {
        if ($ownerName || $businessName || $businessAddress) {
            // First fetch business_line from signup table
            $signupBusinessLine = null;
            ...
            error_log("Business line from signup table: " . ($signupBusinessLine ?: 'null'));
            ...
            if ($upd2) {
                ...
                if ($affected === 0) {
                    ...
                    // We'll also fetch email and business_line from signup table
                    $email = null;
                    $signupBusinessLine = null;
                    if ($emailStmt = $conn->prepare("SELECT email, business_line FROM signup WHERE signup_id = ?")) {
                        ...
                        if ($emailRes && $emailRes->num_rows > 0) {
                            ...
                            $signupBusinessLine = $emailRow['business_line'] ?? null;
                        }
                        $emailStmt->close();
                    }

                    error_log("Inserting new businessowneraccount with business_line: " . ($signupBusinessLine ?: 'null'));
                    ...
                    if ($ins2) {
                        $ins2->bind_param("isssss", $signup_id, $ownerName, $businessName, $businessAddress, $signupBusinessLine, $email);
                        $ins2->execute();
                        $ins2->close();
                    }
                }
